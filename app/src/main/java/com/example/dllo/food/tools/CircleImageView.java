...
/**
 * Created by XiaoyuLu on 16/9/27.
 *
 * 实现自定义 View 将图片变圆形
 */
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (isCircle) {
            ...
            if (drawable != null) {
                ...
                paint.setAntiAlias(true); // 将 画笔 设置为抗齿轮的
                Rect rect = new Rect(0, 0,
                        circleBitmap.getWidth(), circleBitmap.getHeight());
                canvas.drawBitmap(circleBitmap, rect, rect, paint);
            }

        } else {
            // 这个父类方法里有一些实现方法, 可以帮助组件实现
            super.onDraw(canvas);
        }
